...
exports.updateReceipt = async (req, res) => {
  try {
    ...
    else if(userProfile.laundryNumber == receipt.laundryNumber)
    {
      ...
      return res.status(401).json({error: 'This laundry reciept doesn\'t belong to you or you are not a Laundry attendant'});
    if((receipt.studentStatus == true) && (receipt.laundryWorkerStatus == true))
    {
      ...
      return res.json({message: 'The reciept has successfully been deleted due to confirmation of delivery by the student!'})
    } 
    res.status(200).json({
      status: "success",
      data: {
...
exports.createReceipt = async (req, res) => {
  try {
    ...
    if(creatorProfile.role !== 'Laundry attendant')
      return res.status(401).json({errors: 'You are not a Laundry attendant and are not authorised to do this!'})
    const {laundryNumber, torn, clothes} = req.body;
    let receipt = {};
    receipt.laundryNumber = laundryNumber.toUpperCase().trim();
    receipt.creator = req.user.id;
    const studentProfile = await Profile.findOne({laundryNumber: laundryNumber});
